...
def predict_pipeline(lyric):
    """
    Predict script for a single audio sample. Returns probabilities,
    numeric prediction, and string labels consistent with CSV mapping.
    
    Args:
        audio: Path to audio or raw audio array
        csv_id: Optional CSV ID for consistent string labeling
    ...
    lyric_features = np.array(lyric_features)

    # --- Load trained classifier ---
    config = load_config("config/model_config.yml")
    classifier = build_mlp(input_dim=lyric_features.shape[1], config=config)
    model_path = "models/mlp/mlp_multimodal.pth"
    classifier.load_model(model_path)
    classifier.model.eval()

    # --- Run prediction ---
    probability, numeric_prediction, label = classifier.predict_single(lyric_features)
